...
uses
  System.SysUtils, System.Classes, Winapi.Windows, Winapi.Direct3D9;

type
  TColorFormat = (COLOR_R3G3B2, COLOR_R5G6B5, COLOR_X8R8G8B8, COLOR_X1R5G5B5, COLOR_X4R4G4B4, COLOR_A8R8G8B8, COLOR_A1R5G5B5, COLOR_A4R4G4B4, COLOR_A8R3G3B2, COLOR_A2R2G2B2, COLOR_A8, COLOR_UNKNOWN);
...
function PixelXto32(Source: Pointer; SrcFmt: TColorFormat; Mode: integer = 0): Longword;
...
begin
  ...
  case Char1 of
    32..126:
      Result := Char2 = 0;
    129..160: //繁体
      Result := (Char2 in [64..245]);
    161:
      Result := (Char2 in [161..245]);
    162:
      Result := (Char2 in [161..170, 177..226, 229..238, 241..250{252}]);
    163:
      Result := (Char2 in [161..254]);
    164:
      Result := (Char2 in [161..243]);
    165:
      Result := (Char2 in [161..246]);
    166:
      Result := (Char2 in [161..184, 193..216, 224..235, 238..242, 244, 245]);
    167:
      Result := (Char2 in [161..193, 209..241]);
    168:
      Result := (Char2 in [64..126, 128..149, 161..187, {189, 190, }192, 197..233]);
    169:
      Result := (Char2 in [64..90, 92, 96..126, 128..136, 150, 164..239]);
    170..214, 216..254:
      Result := (Char2 in [64..254]);
    215:
      Result := (Char2 in [161..249]);
  end;
end;
...
begin
  case DestFmt of
    COLOR_R3G3B2:
      PByte(Dest)^ := PixelA8R8G8B8_R3G3B2(Source);
    COLOR_R5G6B5:
      PWord(Dest)^ := PixelA8R8G8B8_R5G6B5(Source);
    COLOR_X1R5G5B5:
      PWord(Dest)^ := PixelA8R8G8B8_X1R5G5B5(Source);
    COLOR_X4R4G4B4:
      PWord(Dest)^ := PixelA8R8G8B8_X4R4G4B4(Source);
    COLOR_A1R5G5B5:
      PWord(Dest)^ := PixelA8R8G8B8_A1R5G5B5(Source);
    COLOR_A4R4G4B4:
      PWord(Dest)^ := PixelA8R8G8B8_A4R4G4B4(Source);
    COLOR_A8R3G3B2:
      PWord(Dest)^ := PixelA8R8G8B8_A8R3G3B2(Source);
    COLOR_A2R2G2B2:
      PByte(Dest)^ := PixelA8R8G8B8_A2R2G2B2(Source);
    COLOR_X8R8G8B8:
      PLongword(Dest)^ := Source or $FF000000;
    COLOR_A8R8G8B8:
      PLongword(Dest)^ := Source;
    COLOR_A8:
      PByte(Dest)^ := Source shr 24;
  end;
end;
