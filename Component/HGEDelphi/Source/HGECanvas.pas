...
uses
  ...
  Vcl.Graphics,
  Vcl.Forms,
  HGEBase, HGE, HGETextures, HGEFonts;

const
  bTransparent: array[Boolean] of Integer = (fxNone, fxBlend);

type
  TDXCanvas = class(TObject)
  private
  ...
  public
    ...
    procedure DrawPart(Texture: TDXTexture; X, Y, SrcX, SrcY, Width, Height, ScaleX, ScaleY, CenterX, CenterY: Single; MirrorX, MirrorY: Boolean; Color: Cardinal; BlendMode: Integer); overload;
    procedure DrawPart(Texture: TDXTexture; X, Y, SrcX, SrcY, Width, Height: Single; Color: Cardinal; BlendMode: Integer); overload;
    //圆形
  ...
  public
    ...
    procedure TextOutDraw(X, Y: Integer; Text: WideString; FColor: Cardinal; Alpha: Byte); overload;
    procedure TextOutDraw(X, Y: Integer; Text: WideString; FColor, BColor: Cardinal; Alpha: Byte; boClearMark: Boolean = False); overload;

    procedure TextRect(Rect: TRect; Text: WideString; FColor: Cardinal; TextFormat: TTextFormat = []);

    //字体 自定义颜色
    procedure TextOut(X, Y: Integer; Text: string; FColor: Cardinal; boUnderline: Boolean = False; boBold: Boolean = False); overload;
    //字体 自定义颜色 + 自定义描边
    procedure TextOut(X, Y: Integer; Text: string; FColor: Cardinal; BColor: Cardinal); overload;
    //字体 自定义颜色 + 自定义背景
    procedure TextOut_Bg(X, Y: Integer; Text: string; FColor: Cardinal; BColor: Cardinal);
    //粗体字体 自定义颜色
    procedure TextOutBold(X, Y: Integer; Text: string; FColor: Cardinal; boUnderline: Boolean = False); overload;
    //粗体字体 自定义颜色 + 自定义描边
    procedure TextOutBold(X, Y: Integer; Text: string; FColor: Cardinal; BColor: Cardinal); overload;
    //粗体字体 自定义颜色 + 自定义背景
    procedure TextOutBold_Bg(X, Y: Integer; Text: string; FColor: Cardinal; BColor: Cardinal); overload;
  end;
...
begin
  ...
  FDrawTexture.Format := D3DFMT_A4R4G4B4;
...
procedure TDXCanvas.DrawEx(Texture: TDXTexture; X, Y, CenterX, CenterY, ScaleX, ScaleY: Single; MirrorX, MirrorY: Boolean; Color: Cardinal; BlendMode: Integer);
var
  TempX1, TempY1, TempX2, TempY2: Single;
begin
  SetPattern(Texture.Image, Texture.ClientRect);
...
procedure TDXCanvas.DrawPart(Texture: TDXTexture; X, Y, SrcX, SrcY, Width, Height, ScaleX, ScaleY, CenterX, CenterY: Single; MirrorX, MirrorY: Boolean; Color: Cardinal; BlendMode: Integer);
var
  TexX1, TexY1, TexX2, TexY2: Single;
  TempX1, TempY1, TempX2, TempY2: Single;
begin
...
procedure TDXCanvas.DrawPart(Texture: TDXTexture; X, Y, SrcX, SrcY, Width, Height: Single; Color: Cardinal; BlendMode: Integer);
begin
...
procedure TDXDrawCanvas.TextOut(X, Y: Integer; Text: string; FColor: Cardinal; boUnderline: Boolean; boBold: Boolean);
begin
  //绘制字体效果, 字体颜色
  TextOutDraw(X, Y, Text, FColor, 255);
end;

procedure TDXDrawCanvas.TextOut(X, Y: Integer; Text: string; FColor, BColor: Cardinal);
begin
  //绘制字体效果, 字体颜色 + 描边颜色
  TextOutDraw(X, Y, Text, FColor, BColor, 255);
end;

procedure TDXDrawCanvas.TextOut_Bg(X, Y: Integer; Text: string; FColor, BColor: Cardinal);
begin
  //绘制字体效果, 字体颜色 + 背景颜色
  TextOutDraw(X, Y, Text, FColor, BColor, 255, True);
end;

procedure TDXDrawCanvas.TextOutBold(X, Y: Integer; Text: string; FColor: Cardinal; boUnderline: Boolean);
begin
  //绘制粗体字体效果, 字体颜色
  TextOutDraw(X, Y, Text, FColor, 255);
end;

procedure TDXDrawCanvas.TextOutBold(X, Y: Integer; Text: string; FColor, BColor: Cardinal);
begin
  //绘制粗体字体效果, 字体颜色 + 描边颜色
  TextOutDraw(X, Y, Text, FColor, 255);
end;

procedure TDXDrawCanvas.TextOutBold_Bg(X, Y: Integer; Text: string; FColor: Cardinal; BColor: Cardinal);
begin
  //绘制粗体字体效果, 字体颜色 + 背景颜色
  TextOutDraw(X, Y, Text, FColor, 255);
end;

procedure TDXDrawCanvas.TextOutDraw(X, Y: Integer; Text: WideString; FColor, BColor: Cardinal; Alpha: Byte; boClearMark: Boolean);
var
  Texture: TDXTexture;
  AsciiRect: TRect;
begin
  ...
  Dec(Y);
  //改变纹理大小
  Texture.Active := False; //更新纹理大小需要设置为 False
  Texture.Size := Point(TextWidth(Text) + 4, TextHeight(Text) + 4);
  Texture.PatternSize := Point(TextWidth(Text) + 4, TextHeight(Text) + 4);
  Texture.Active := True; //更新完成后 设置为True
  Texture.Clear;
  Texture.TextOutTexture(2, 1, Text, FColor, BColor, boClearMark);

  AsciiRect.Left := 0;
  AsciiRect.Top := 0;
  AsciiRect.Right := TextWidth(Text) + 3;
  AsciiRect.Bottom := TextHeight(Text) + 2;

  //背景
  if boClearMark then
    Rectangle(x, y, AsciiRect.Right, AsciiRect.Bottom, DisplaceRB(BColor or $FF000000), True);

  Draw(x, y, Texture.ClientRect, Texture, True, cColor4($FFFFFF or (Alpha shl 24 and $FF000000)));
end;

procedure TDXDrawCanvas.TextOutDraw(X, Y: Integer; Text: WideString; FColor: Cardinal; Alpha: Byte);
var
  i: integer;
  sWord: Word;
  Texture: TDXTexture;
...
begin
  ...
  for I := 1 to Length(Text) do begin
    ...
    if (AsciiRect.Right > 4) then begin
      DrawRect(Texture, X + nWidth, Y - 1, AsciiRect.Left, AsciiRect.Top, AsciiRect.Right, AsciiRect.Bottom, fxBlend, (Alpha shl 24) or FColor);
