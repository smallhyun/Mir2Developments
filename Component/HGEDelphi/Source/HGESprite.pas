...
constructor THGESprite.Create(const Texture: ITexture; const TexX, TexY, W, H: Single);
var
...
function THGESprite.GetBoundingBox(const X, Y: Single; var Rect: THGERect): PHGERect;
begin
  Rect := HGErct.SetRect(X - FHotX, Y - FHotY, X - FHotX + FWidth, Y - FHotY + FHeight);
  Result := @Rect;
end;

function THGESprite.GetBoundingBoxEx(const X, Y, Rot, HScale, VScale: Single; var Rect: THGERect): PHGERect;
var
...
begin
  ...
  if (TW <> FTexWidth) or (TH <> FTexHeight) then
  begin
...
procedure THGESprite.SetTextureRect(const X, Y, W, H: Single; const AdjSize: Boolean = True);
var
