...
uses
	Classes, SysUtils, WinTypes, StrUtils, WinProcs, Graphics, Messages, Dialogs;
...
// ÀÏ¹Ý ½ºÆ®¸µÀ» Äõ¸®¿¡ ÀûÇÕÇÏ°Ô º¯°æ : PDS
function StrToSQLSafe  ( str : string ):string;
// Äõ¸®½ºÆ®¸µÀ» ÀÏ¹Ý ½ºÆ®¸µÀ¸·Î º¯°æ : PDS
function SQLSafeToStr  ( sqlstr : string ):string;
// °³Çà¹®ÀÚµîÀ» »èÁ¦ÇÑ ½ºÆ®¸µÀ¸·Î º¯°æ
...
begin
(*
  1¡¢s := 'Ò»¶þÈý{²âÊÔ²âÊÔ|251:249:1}ËÄÎåÁù';
  sMsg := ArrestStringEx(s, '{', '}', sMsg2);
  sMsg = ËÄÎåÁù
  sMsg2 = ²âÊÔ²âÊÔ|251:249:1
  2¡¢s2 := '{²âÊÔ²âÊÔ|251:249:1}Ò»¶þÈý{²âÊÔ²âÊÔ|251:249:1}ËÄÎåÁù';
  sMsg := ArrestStringEx(s2, '{', '}', sMsg2);
  sMsg = Ò»¶þÈý{²âÊÔ²âÊÔ|251:249:1}ËÄÎåÁù
  sMsg2 = ²âÊÔ²âÊÔ|251:249:1
  3¡¢s3 := '{²âÊÔ²âÊÔ|251:249:1}Ò»¶þÈý{²âÊÔ²âÊÔ|251:249:1}ËÄÎåÁù{²âÊÔ²âÊÔ|251:249:1}';
  sMsg := ArrestStringEx(s3, '{', '}', sMsg2);
  sMsg = Ò»¶þÈý{²âÊÔ²âÊÔ|251:249:1}ËÄÎåÁù{²âÊÔ²âÊÔ|251:249:1}
  sMsg2 = ²âÊÔ²âÊÔ|251:249:1
  4¡¢s4 := '{Ò»¶þÈý{²âÊÔ²âÊÔ|251:249:1}ËÄÎåÁù';
  sMsg := ArrestStringEx(s4, '{', '}', sMsg2);
  sMsg = ËÄÎåÁù
  sMsg2 = Ò»¶þÈý{²âÊÔ²âÊÔ|251:249:1
*)
  ...
  try
    ...
    if i > 0 then begin                 //ÓÐÇ°×º
      n := posEx(ArrestBefore, Source, i + 1);
      if n > i then begin               //ÓÐºó×º
        ArrestStr := Copy(Source, i + 1, n - i - 1); //È¡Ç°ºó×ºÖ®¼äµÄ×Ö·û´®
        Result := Copy(Source, n + 1, SrcLen - n); //Ê£ÓàÊý¾ÝÈ¡ºó×ºÖ®ºóµÄ×Ö·û´®
      end else begin                    //ÓÐÇ°×ºÃ»ºó×º
        ArrestStr := Copy(Source, i + 1, SrcLen - i); //Ç°×ºÖ®ºóµÄ×Ö·û´®¶¼ÊÇArrestStr
        Result := '';                   //Ê£ÓàÊý¾ÝÎª¿Õ
      end;
    end else begin                      //Ã»ÓÐÇ°×º
      n := Pos(ArrestBefore, Source);
      if n > 0 then begin               //Ã»Ç°×ºÓÐºó×º
        ArrestStr := Copy(Source, 1, n - 1); //È¡ºó×ºÖ®Ç°µÄ×Ö·û´®
        Result := Copy(Source, n + 1, SrcLen - n) //Ê£ÓàÊý¾ÝÎªºó×ºÖ®ºóµÄ×Ö·û´®
      end else begin                    //Ã»Ç°×ºÒ²Ã»ºó×º
        ArrestStr := Source;            //È¡È«²¿Êý¾Ý
        Result := '';                   //Ê£ÓàÊý¾ÝÎª¿Õ
      end;
    end;
    //---------------Ëã·¨ÓÅ»¯½áÊø------------------------
