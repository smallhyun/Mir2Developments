...
//         if BoTestServer then           //Èç¹ûÊÇÌåÑéÄ£Ê½±¬ÂÊ5±¶         
//            RealMaxPoint := Random(_MAX(1, pmi.MaxPoint div 5))  //µå·ÓÀ² 5¹è ÀÌº¥Æ®(Å×¼·)
//         else
...
//               mon.Gold := mon.Gold + (pmi.Count div 2) + Random(pmi.Count);
               ...
               if CopyToUserItemFromName (iname, pu^) then begin
                  ...
                  if Random(7) = 1 then    //¹ÖÎï±¬ÂÊ¼«Æ·¼¸ÂÊ£¬Ô­À´ÊÇif Random(10) = 2 then
...
if (pz.MonRace = RC_PBKING) then begin
   ...
   if Result = TRUE then begin
      if (GetMonLevel( pz.MonName ) = 70) or
            ...
            (pz.MonName = 'Èæ»ç¿Õ') or (pz.MonName = '±Íµ·¿Õ') or (pz.MonName = 'ÈæÃµ¸¶¿Õ') or
            (pz.MonName = 'ÆÄÈ²¸¶½Å') then begin
         MainOutMessage('¿Õ¸÷Á¨: ' + pz.MonName + ' (' + pz.MapName + ') ' + TimeToStr(Now));
      end;
   end;
end;
...
begin
   ...
   for i:=0 to RunUserList.Count-1 do begin
      if CompareText (TUserHuman(RunUserList.Objects[i]).UserId, uid) = 0 then begin
         Result := TUserHuman(RunUserList.Objects[i]).SetExpiredTime( 5 * 60 + 1 ); // ÃÊ ÀÔ·Â
         break;
      end;
   end;
end;
      ...
      try
         ...
         if pshift = nil then begin //¼­¹ö ÀÌµ¿ÀÌ ¾Æ´Ô
            ...
            if mapenvir <> nil then begin
               ...
               if mapenvir.Fight3Zone then begin  //¹®ÆÄ ´ë·Ã ÀÌº¥Æ® ¹æ¿¡ ÀÖÀ½.
                  //Á×Àº °æ¿ì
